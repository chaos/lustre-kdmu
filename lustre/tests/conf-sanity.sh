...
#                                  dmu dmu
ALWAYS_EXCEPT="$CONF_SANITY_EXCEPT 5d  55"
...
test_36() { # 12743
	...
	start fs2ost $fs2ostdev $OST_MOUNT_OPTS $WCF
	start fs3ost $fs3ostdev $OST_MOUNT_OPTS $WCF
	mkdir -p $MOUNT2
	mount -t lustre $MGSNID:/${FSNAME2} $MOUNT2 || return 1
...
test_46a() {
	...
	for (( i=2; i<=$OSTCOUNT; i++ )); do
		start ost$i `ostdevname $i` $OST_MOUNT_OPTS $WCF || return $((i+2))
	done
...
test_56() {
	...
	start_mds $WCF
	start_ost $WCF
	start_ost2 $WCF || error "Unable to start second ost"
	mount_client $MOUNT || error "Unable to mount client"
	echo ok
	$LFS osts
...
test_59() {
	...
	start_mgsmds $WCF >> /dev/null || error "MDT start failed"
	local C2=$(count_osts)
	...
	start_ost $WCF >> /dev/null || error "OST1 start failed"
	local C3=$(count_osts)
	echo "after ost writeconf count: $C3 (expect 1)"
	[ $C3 -eq 1 ] || error "new OST writeconf should add:"
	start_ost2 $WCF >> /dev/null || error "OST2 start failed"
	local C4=$(count_osts)
