...
static void *quota_start(struct seq_file *p, loff_t *pos)
{
        return p->private;
}

static int quota_show(struct seq_file *f, void *it)
{
        ...
        if (rc == 0)
                return seq_printf(f, LPU64"\t\t"LPU64"\n",
...
static void *quota_next(struct seq_file *f, void *it, loff_t *pos)
{
        struct dt_object       *dt   = DT_IT2DT(it);
        const struct dt_it_ops *iops = &dt->do_index_ops->dio_it;
        int rc;
...
static void quota_stop(struct seq_file *f, void *v)
{
        /* Nothing to do */
}

static struct seq_operations quota_sops = {
        .start                 = quota_start,
        .stop                  = quota_stop,
        .next                  = quota_next,
        .show                  = quota_show,
...
static int quota_seq_open(struct inode *inode, struct file *filp, int isgroup)
{
        struct proc_dir_entry   *dp  = PDE(inode);
        struct osd_device       *osd = dp->data;
        struct seq_file         *seq;
...
static int quota_user_seq_open(struct inode *inode, struct file *filp)
{
        return quota_seq_open(inode, filp, ISUSER);
}

static int quota_group_seq_open(struct inode *inode, struct file *filp)
{
        return quota_seq_open(inode, filp, ISGROUP);
}

static int quota_seq_release(struct inode *inode, struct file *filp)
{
...
static struct file_operations quota_user_file_ops = {
        .owner   = THIS_MODULE,
...
static struct file_operations quota_group_file_ops = {
        .owner   = THIS_MODULE,
