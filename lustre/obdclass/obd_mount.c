...
/* Get lsi from svname */
static struct lustre_sb_info *server_find_mount(const char *name)
{
        ...
        cfs_list_for_each_entry(lsi, &lsi_list, lsi_list_chain) {
                if (strcmp(name, lsi->lsi_svname) == 0)
                        RETURN(lsi);
                if ((strcmp(name, LUSTRE_MGS_OBDNAME) == 0) &&
                    IS_MGS(lsi))
                        RETURN(lsi);
        }
        RETURN(NULL);
...
struct lustre_sb_info *server_find_mount_locked(const char *name)
{
        struct lustre_sb_info *lsi;

        cfs_down(&lsi_list_lock);
        lsi = server_find_mount(name);
...
/** Take a reference on the lsi using the svname.
 * This is just for initial obd setup to find the mount struct.
 * It should not be called every time you want to mntget.
 */
...
int server_put_mount(const char *name)
{
        ...
        CDEBUG(D_MOUNT, "%s: put refs=%d\n",
               name, cfs_atomic_read(&lsi->lsi_mounts));

        if (lustre_put_lsi(lsi))
                CDEBUG(D_MOUNT, "%s: last put\n", name);
...
static int server_stop_servers(int lsiflags)
{
        ...
        if ((lsiflags & LDD_F_SV_TYPE_OST) &&
            (obd = class_name2obd(LUSTRE_OSS_OBDNAME))) {
                /*FIXME pre-rename, should eventually be LUSTRE_OSS_NAME*/
...
/* FIXME - add new mount opt PARAM_NETWORK, but we should probably
   filter them out when we first collect nids intead of here. */
        ...
        strncpy(mti->mti_svname, lsi->lsi_svname, MTI_NAME_MAXLEN);
...
int server_kernel_mount(struct lustre_sb_info *lsi, unsigned long mflags)
{
         ...
         * writeconf, mgs, iam FIXME?, anything else?
         */
        ...
        /* FIXME do we want to check/set anything here? */
        LASSERT(dt_param.ddp_mount_type);
...
static int lustre_server_mount(struct lustre_sb_info *lsi, unsigned long mflags)
{
        ...
        /* the One True Mount, populating the rest of our lsi.
           If this returns without error, the OSD has started. */
        rc = server_kernel_mount(lsi, mflags);
        if (rc) {
...
void lustre_server_umount(struct lustre_sb_info *lsi)
{
        ...
        /* Might still be running in error cases */
        stop_osd(lsi);
