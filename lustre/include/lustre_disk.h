...
/* lustre_disk_data used to be persistent on disk; for DMU all this
 * information needs to be provided at mount time.  For ldiskfs we can
 * secretly still read the persistent data with mount.lustre, then
 * convert it to mount line options.
 * As such, it has moved into mount_utils.h
 */

/* LDD_* and LSI_* flags are shared */
/* FIXME these flags should all be removed / moved int mount_utils */
