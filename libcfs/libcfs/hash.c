 ...
 * - "bucket" is a group of cfs_hlist_head now, user can speicify bucket size
 *   by bkt_bits of cfs_hash_create(), all cfs_hlist_heads in a bucket share
 ...
 *
 * - support both cfs_spin_lock/rwlock for bucket:
                      ...
                      cfs_hlist_node_t *hnd)
{
        ...
        if (hnd->next == NULL) { /* it's the tail */
                dh->dh_tail = (hnd->prev == (cfs_hlist_node_t *)&dh->dh_head) ?
                              NULL : hnd->prev;
        }
                      ...
                      cfs_hlist_node_t *hnd)
{
        ...
        if (hnd->next == NULL) { /* it's the tail */
                dh->dd_tail = (hnd->prev == (cfs_hlist_node_t *)&dh->dd_head) ?
                              NULL : hnd->prev;
        }
        cfs_hlist_del_init(hnd);
        return --dh->dd_depth;
...
cfs_hash_for_each_relax(cfs_hash_t *hs, cfs_hash_for_each_cb_t func, void *data)
{
        ...
        cfs_hash_for_each_bucket(hs, &bd, i) {
                ...
                cfs_hash_bd_for_each_hlist(hs, &bd, hhead) {
                        for (hnode = hhead->next; hnode != NULL;) {
                                cfs_hash_bucket_validate(hs, &bd, hnode);
