...
{
        /* register sysctl table into params_tree */
        libcfs_param_sysctl_init("openibnal", libcfs_param_kibnal_ctl_table,
...
{
        libcfs_param_sysctl_fini("openibnal", libcfs_param_lnet_root);
#if defined(CONFIG_SYSCTL) && !CFS_SYSFS_MODULE_PARM
        if (kibnal_tunables.kib_sysctl != NULL)
